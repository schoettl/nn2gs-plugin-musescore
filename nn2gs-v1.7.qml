...
import QtQuick
import MuseScore
//import Muse.UiComponents // it redefines some components like CheckBox but I use more features than provided there
//import QtQuick.Dialogs // not working on Windows in MS 4.4
    ...
    if (c.readyState == XMLHttpRequest.DONE) {
      if (console.log("HTTP status: " + c.status), 200 == c.status) {
        ...
        try {
          ...
          console.error("Invalid JSON response. Could not parse."), messagePane.showError("Ung\u00fcltige Antwort vom Server.");
        }
        d(a, lastResults);
        ...
        messagePane.showError(`Unbekannter Netzwerkfehler (HTTP Status Code ${c.status}). Funktioniert das Internet? Funktioniert ${apiUrl}?\n\n${e}`);
        console.error(e);
      }
    } else {
...
function changeNotes(a, b) {
  return function(d, f) {
    ...
    if (d.length != f.length) {
      console.error(`Length of selected chords (${d.length}) and translation result (${f.length}) do not match. This is likely a bug. Aborting and invalidating current translation result.`), invalidateCurrentResults();
    } else {
  ...
  e.onreadystatechange = function() {
    if (e.readyState == XMLHttpRequest.DONE) {
      if (console.log("HTTP status: " + e.status), 200 == e.status) {
        ...
        messagePane.showError(`Unbekannter Netzwerkfehler (HTTP Status Code ${e.status}). Funktioniert das Internet? Funktioniert ${apiUrl}?\n\n${g}`);
        console.error(g);
      }
    } else {
...
function addLyricsToNotes(a) {
  return function(b, d) {
    ...
    if (b.length != d.length) {
      ...
      curScore.endCmd();
      f && messagePane.showError(`${f} Note(n) konnten nicht \u00fcbersetzt werden und wurden rot markiert.\n\nEntweder existieren sie nicht auf dem Instrument oder sie waren bereits rot markiert. Die Akkorde mit roten Noten wurden bei der \u00dcbersetzung \u00fcbersprungen.`);
    }
  };
}
function addBassNamesAsLyrics(a) {
  ...
  let b = collectChords();
  0 == b.length ? (console.warn("Keine Noten ausgew\u00e4hlt. Abbruch."), messagePane.showWarning("Es sind keine Noten bzw. Takte ausgew\u00e4hlt.")) : b.length > maxChordLimit ? (console.warn("Zu viele Noten ausgew\u00e4hlt. Abbruch."), messagePane.showWarning("Es sind zu viele Noten bzw. Takte ausgew\u00e4hlt. Es k\u00f6nnen immer nur ein paar Takte auf einmal \u00fcbersetzt werden.")) : (isCurrentResultValid() ? (alternativeIndex = a !== lastZD ? 0 : alternativeIndex + 1, addLyricsToNotes(a)(b, 
  lastResults)) : (alternativeIndex = 0, callBassApi(b, addLyricsToNotes(a)), btnNextAlternative.enabled = !0), invalidateResultsAfterTimeout());
}
function translateToFromGriffschrift(a) {
  ...
  let d = collectChords();
  if (0 == d.length) {
    console.warn("Keine Noten ausgew\u00e4hlt. Abbruch."), messagePane.showWarning("Es sind keine Noten bzw. Takte ausgew\u00e4hlt.");
  } else {
    if (d.length > maxChordLimit) {
      console.warn("Zu viele Noten ausgew\u00e4hlt. Abbruch."), messagePane.showWarning("Es sind zu viele Noten bzw. Takte ausgew\u00e4hlt. Es k\u00f6nnen immer nur ein paar Takte auf einmal \u00fcbersetzt werden.");
    } else {
...
function checkVoiceCheckboxesValidity() {
  checkBoxVoice1.checked || checkBoxVoice2.checked || checkBoxVoice3.checked || checkBoxVoice4.checked || messagePane.showWarning("Mindestens eine Stimme muss zum \u00dcbersetzen ausgew\u00e4hlt sein.");
}
...
function checkMuseScoreVersionSupport() {
  (4 > mscoreMajorVersion || 4 <= mscoreMajorVersion && 3 >= mscoreMinorVersion) && messagePane.showError(`Diese Version des Plugins funktioniert nur mit MuseScore 4.4 und h\u00f6her. Sie verwenden gerade MuseScore ${mscoreMajorVersion}.${mscoreMinorVersion}.`);
}
    ...
    function clickReverseDirection() {
        ...
        btnReverseDirection.checked = (state === 2)
    }
        ...
        ColumnLayout {
            ...
            Pane {
                    ...
                    ColumnLayout {
                        ...
                        Rectangle {
                            ...
                            Text {
                                id: titleLabel
                                visible: false
                                // Without layout, both labels are overlapping.
                            }
                            Text {
                                id: messageLabel
                                text: "test text should be wrapped automatically at some place"
                                // NOTE: Text wrapping just doesn't work, only if Text/Label is inside a Rectangle
                                /* clip: false */
                ...
                function showInfo() {
                    ...
                    messageLabel.text = "Nn2GS - Übersetzen zwischen Normalnoten und Griffschrift für Steirische Harmonika und ähnliche Instrumente.\n\nhttps://griffschrift-notation.de/\n\nMomentan kostenlos, irgendwann wird man für die Nutzung vielleicht eine günstige Lizenz kaufen müssen.\n\nTastenkürzel: Alt+R (Übersetzungsrichtung), Alt+J (Zug), Alt+K (Druck); auch auf Beschriftungen im Plugin kann geklickt werden (versteckte Funktionen)."
                    open()
                }
                function showWarning(msg) {
                    titleLabel.text = "Warnung"
            ...
            SpinBox {
                ...
                valueFromText: function(text, locale) {
                    return findTonartIndex(tonarten, text.toUpperCase())
                }
                onValueChanged: {
                    invalidateCurrentResults()
                }
            ...
            Button {
                text: "Info"
                Layout.fillWidth: true
                onClicked: messagePane.showInfo()
            }
        }
    }
}
